...
if [ "$ZYNQ_TYPE" == "zynqmp" ] ; then
	...
	IMG_NAME="uImage"
	ARCH=arm
    LINUX_OUT=../out-adi-zynq
	#DTDEFAULT=zynq-zc702-adv7511-ad9361-fmcomms2-3.dtb
	DTDEFAULT=zynq-zc706-adv7511-adrv9009.dtb
fi
...
make $DEFCONFIG O=$LINUX_OUT

make -j$NUM_JOBS $IMG_NAME UIMAGE_LOADADDR=0x8000 O=$LINUX_OUT
...
make $DTFILE O=$LINUX_OUT

popd 1> /dev/null

ls -l $LINUX_OUT/arch/$ARCH/boot/$IMG_NAME .
ls -l $LINUX_OUT/arch/$ARCH/boot/dts/$DTFILE .
