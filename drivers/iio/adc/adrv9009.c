...
Notes:
	it's generated by TES-GUI -> "Tools" -> "Init C Files"
	Then it can be convert into a header file for non-OS platform like below 
...
static int adrv9009_txlol_ecal(struct adrv9009_rf_phy *phy)
{
	...
	//devmem 0x43C50000 32 0x0 // default is 0/ARM-control
	//writel(0x80000000, phy->tddc_regs + 0x00);
	dev_info(&phy->spi->dev, "%s: inw(0x43C50000)=0x%08x", __func__, readl(phy->tddc_regs + 0x00));
#endif
	...
	/* 20190529: send signal with 24dbm, it's a must per manually test */
	TALISE_setTxAttenuation(phy->talDevice, TAL_TX1, 16);
...
#else
	//devmem 0x43C500BC 32 0x0554fc08 # ECAL for UA_TX1<->UA_ORX2
	writel(0x0554fc08, phy->tddc_regs + 0xBC);
	dev_info(&phy->spi->dev, "%s: inw(0x43C500BC)=0x%08x", __func__, readl(phy->tddc_regs + 0xBC));
#endif
	...
	ret = TALISE_getTxLolStatus(phy->talDevice, TAL_TX1, &txLolStatus);
	if (ret != TALACT_NO_ACTION) {
		dev_err(&phy->spi->dev, "%s:%d (ret %d)", __func__, __LINE__, ret);
		goto txlol_ecal_abort;
	}
	dev_info(&phy->spi->dev, "%s: TALISE_getTx1LolStatus: error=0x%X "
		...
		txLolStatus.varianceMetric, txLolStatus.iterCount,
		txLolStatus.updateCount);
#endif
	/* page174.step9: Turn on PA2 and switch TX2 to ORX2: FHK-RFFC has ORX2 only */
	/*** < Action: Please ensure PA is enabled operational at this time > ***/
	/* 20190529: send signal with 24dbm, it's a must per manually test */
	TALISE_setTxAttenuation(phy->talDevice, TAL_TX2, 16);
#ifndef RADIO_CTL_PIN_MODE
	ret = TALISE_setRxTxEnable(phy->talDevice,
				   has_rx_and_en(phy) ? TAL_ORX2_EN : 0,
...
#else
	writel(0x0554bc0c, phy->tddc_regs + 0xBC);
	dev_info(&phy->spi->dev, "%s: inw(0x43C500BC)=0x%08x",
		__func__, readl(phy->tddc_regs + 0xBC));
#endif
	...
	ret = TALISE_getTxLolStatus(phy->talDevice, TAL_TX2, &txLolStatus);
	if (ret != TALACT_NO_ACTION) {
		dev_err(&phy->spi->dev, "%s:%d (ret %d)", __func__, __LINE__, ret);
		goto txlol_ecal_abort;
	}
...
#else
	writel(0x04aa03f0, phy->tddc_regs + 0xBC);
	dev_info(&phy->spi->dev, "%s: inw(0x43C500BC)=0x%08x",
		__func__, readl(phy->tddc_regs + 0xBC));
#endif
	/* FPGA-RFES-TDDC: io_config_manually disable */
	//devmem 0x43C3002C 32 0x0
	//writel(0x00000000, phy->tddc_regs + 0x2C);
	//dev_info(&phy->spi->dev, "%s: inw(0x43C3002C)=0x%08x",
	//	__func__, readl(phy->tddc_regs + 0x2C));
txlol_ecal_release:
...
static int adrv9009_do_setup(struct adrv9009_rf_phy *phy)
{
	...
	/*************************************************************************/
	/***** < User Stop SYSREF Here BEFORE Enable ALL Framers/Deframers > *****/
	/***** Enable Talise JESD204B Framers & Deframers                    *****/
	/***** < User Send SYSREF Here AFTER  Enable ALL Framers/Deframers > *****/
	/***** Check  Talise JESD204B Framers & Deframers                    *****/
	/*************************************************************************/
	...
	//FpgaTalise.EnableJesd204bDeframer(FpgaTalise.FpgaDeframerSelect.DeframerAandB)
	...
	if (!IS_ERR_OR_NULL(phy->jesd_rx_clk) && phy->talInit.jesd204Settings.framerA.M) {
		...
		/* JACKY-20190530: to align with TES-python
		ret = TALISE_enableFramerTestData(phy->talDevice, TAL_FRAMER_A,
			TAL_FTD_ADC_DATA, TAL_FTD_FRAMERINPUT);
		if (ret != TALACT_NO_ACTION) {
			dev_err(&phy->spi->dev, "%s:%d (ret %d)", __func__, __LINE__, ret);
			ret = -EFAULT;
			goto out_disable_tx_clk;
	...
	if (!IS_ERR_OR_NULL(phy->jesd_rx_os_clk) && phy->talInit.jesd204Settings.framerB.M) {
		...
		/* JACKY-20190530: to align with TES-python
		ret = TALISE_enableFramerTestData(phy->talDevice, TAL_FRAMER_B,
			TAL_FTD_ADC_DATA, TAL_FTD_FRAMERINPUT);
		if (ret != TALACT_NO_ACTION) {
			dev_err(&phy->spi->dev, "%s:%d (ret %d)", __func__, __LINE__, ret);
			ret = -EFAULT;
	...
	//FpgaTalise.ResetFpgaIp(FpgaTalise.FpgaResets.ClearAllRst)
	//FpgaTalise.EnableJesd204bDeframer(FpgaTalise.FpgaDeframerSelect.DeframerA)
	...
	/*** JESD204B Sync Verification ***/

	/** Check Talise DeframerA Status **/
	if (!IS_ERR_OR_NULL(phy->jesd_tx_clk) && phy->talInit.jesd204Settings.deframerA.M) {
	...
	//phy->tracking_cal_mask = TAL_TRACK_RX1_QEC | TAL_TRACK_RX2_QEC | TAL_TRACK_TX1_QEC | TAL_TRACK_TX2_QEC;
...
#if 0 /* disable to keep ORX2<->TX2 for Observer the effect after TxLOL_ECAL */
	/* set RFFC+RFIC into Rx state */
#ifndef RADIO_CTL_PIN_MODE
	ret = TALISE_setRxTxEnable(phy->talDevice,
				   has_rx_and_en(phy) ? TAL_RX1RX2_EN : 0,
	...
	dev_info(&phy->spi->dev, "%s: inw(0x43C30058)=0x%08x",
		__func__, readl(phy->tddc_regs + 0x58));
#endif
#endif
