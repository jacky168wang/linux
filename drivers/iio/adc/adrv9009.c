...
static int adrv9009_txlol_ecal(struct adrv9009_rf_phy *phy)
{
	...
	ret = TALISE_setRadioCtlPinMode(phy->talDevice,
		TAL_TXRX_PIN_MODE | TAL_ORX_PIN_MODE, TAL_ORX1ORX2_PAIR_89_SEL);
	/* to prove/verify "TX1/2_ENABLE" connection is correct or not */
	//ret = TALISE_setRadioCtlPinMode(phy->talDevice,
		//TAL_TXRX_PIN_MODE, TAL_ORX1ORX2_PAIR_NONE_SEL);
	//ret = TALISE_setRadioCtlPinMode(phy->talDevice,
				 ...
				 long m)
{
	...
	case IIO_CHAN_INFO_HARDWAREGAIN:
		if (chan->output) {
			...
			dev_dbg(&phy->spi->dev, "%s: Tx2Attenuation %d mdB", __func__, atten_mdb);
			*val = -1 * (atten_mdb / 1000);
			*val2 = (atten_mdb % 1000) * 1000;
			if (!*val)
				*val2 *= -1;
			dev_dbg(&phy->spi->dev, "%s: val=%d val2=%d mask=%ld", __func__, *val, *val2, m);
		} else {
			u8 index;
				  ...
				  long mask)
{
	...
	case IIO_CHAN_INFO_HARDWAREGAIN:
		if (chan->output) {
			...
			dev_dbg(&phy->spi->dev, "%s: val=%d val2=%d mask=%ld", __func__, val, val2, mask);
			code = ((abs(val) * 1000) + (abs(val2) / 1000));
			dev_dbg(&phy->spi->dev, "%s: Tx2Attenuation %d mdB", __func__, code);
